"""npmi_matrix.py — Co-occurrence counts → sparse NPMI association matrix.

Ownership: core/engine/training/npmi_matrix.py
    Owns NPMI normalisation and construction of the positive association
    matrix AND the inhibitory contradiction edge set.
...
Responsibilities:
    ...
    - Capture strong negative NPMI < inhibit_threshold as InhibitEdge records
    - Return a symmetric scipy.sparse.csr_matrix (V × V)

Law of Contradiction (Builder Contract §2.1):
    Negative NPMI scores must NOT be clamped to 0.
    Any NPMI < inhibit_threshold (default -0.8) MUST be captured and
    returned as InhibitEdge records to populate the Contradiction Pressure
    metric in the Hot Envelope.
...
V1 reference: _BDHyperNeuronEMITTER/src/engine/training/npmi_matrix.py
V1 changes (none — math preserved identically):
...
# HUMAN-IN-THE-LOOP PERMISSION.  This module defines the association matrix
# used for spectral factorisation and the inhibit edges used for Contradiction
# Pressure.  Alterations here impact determinism and semantic fidelity across
# the entire embedding pipeline.
"""
...
class InhibitEdge(NamedTuple):
    """A strong contradiction edge captured from negative NPMI.

    Stored in the Cold Artifact and loaded into the GraphBLAS
    inhibition matrix H at query time.
    """
    token_a: int
    token_b: int
    weight: float   # abs(npmi), always positive, always >= abs(inhibit_threshold)


@dataclass
class NPMIResult:
    """Combined output of build_npmi_matrix_with_inhibits()."""
    matrix: csr_matrix          # positive association matrix (V x V)
    inhibit_edges: List[InhibitEdge]   # strong contradiction pairs
...
def build_npmi_matrix_with_inhibits(
    pair_counts: Dict[Tuple[int, int], float],
    token_counts: Dict[int, int],
    vocab_size: int,
    *,
    smoothing: float = 0.0,
    inhibit_threshold: float = -0.8,
) -> NPMIResult:
    """Build the positive association matrix AND capture strong inhibit edges.

    Per the Law of Contradiction (Builder Contract §2.1):
    NPMI values below ``inhibit_threshold`` (default -0.8) are captured as
    InhibitEdge records rather than clamped to zero.  These edges populate
    the GraphBLAS inhibition matrix H in the Hot Envelope.

    Parameters
    ----------
    pair_counts, token_counts, vocab_size, smoothing :
        Same as ``build_npmi_matrix``.
    inhibit_threshold : float
        NPMI cutoff for contradiction edges.  Must be negative (default -0.8).
        Any pair with NPMI < threshold is captured as an InhibitEdge.

    Returns
    -------
    NPMIResult
        ``.matrix``        — positive-only NPMI CSR matrix (for SVD)
        ``.inhibit_edges`` — list of InhibitEdge(token_a, token_b, weight)
    """
    if inhibit_threshold >= 0:
        raise ValueError("inhibit_threshold must be negative (e.g. -0.8)")
    ...
    if total_tokens == 0 or total_pairs == 0:
        return NPMIResult(
            matrix=csr_matrix((vocab_size, vocab_size), dtype=float),
            inhibit_edges=[],
        )

    pos_matrix = dok_matrix((vocab_size, vocab_size), dtype=float)
    inhibit_edges: List[InhibitEdge] = []

    p_a_cache: Dict[int, float] = {
        tok: cnt / total_tokens
        for tok, cnt in token_counts.items()
        if cnt > 0
    }
    ...
    for (a, b), count_ab in pair_counts.items():
        ...
        p_ab = (count_ab + smoothing) / (
            total_pairs + smoothing * vocab_size * vocab_size
        )
        p_a = p_a_cache.get(a, 0.0)
        p_b = p_a_cache.get(b, 0.0)
        ...
        if npmi > 0.0:
            # Positive association → excitatory matrix
            pos_matrix[a, b] = npmi
            pos_matrix[b, a] = npmi
        elif npmi < inhibit_threshold:
            # Strong contradiction → inhibition edge (weight is positive magnitude)
            inhibit_edges.append(InhibitEdge(a, b, abs(npmi)))

    return NPMIResult(matrix=pos_matrix.tocsr(), inhibit_edges=inhibit_edges)

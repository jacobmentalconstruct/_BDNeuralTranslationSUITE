"""bpe_trainer.py — Deterministic subword vocabulary builder (training-time only).

Ownership: core/engine/training/bpe_trainer.py
    Owns BPE merge training, corpus ingestion, and tokenizer.json
    serialisation.  Training-time only — produces the vocab and merge list
    consumed by the inference provider at query time.
...
V1 reference: _BDHyperNeuronEMITTER/src/engine/training/bpe_trainer.py
...
class BPETrainer:
        ...
        for root, _, files in os.walk(str(corpus_dir)):
            for fname in sorted(files):  # sorted for determinism
                ...
                try:
                    ...
                    continue
        log.debug("BPETrainer: loaded %d word tokens from %s", len(words), corpus_dir)
        return words
        ...
        for word in words:
            for symbol in word:
                if symbol not in vocab:
                    ...
                    next_id += 1
        self._vocab = vocab
        log.debug("BPETrainer: initial vocab size = %d", len(self._vocab))
    ...
    def train(self, corpus_dir: str | Path) -> None:
        ...
        log.info(
            "BPETrainer.train(): final vocab_size=%d merges=%d",
    ...
    def save(self, path: str | Path) -> None:
        ...
        log.info("BPETrainer.save(): wrote tokenizer to %s", dest)
    ...
    def load(self, path: str | Path) -> None:
        ...
        log.debug("BPETrainer.load(): vocab_size=%d", len(self._vocab))
